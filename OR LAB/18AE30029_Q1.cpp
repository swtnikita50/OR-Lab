...
void solve_with_dual_simplex(vvd tableau, vd objective_fn, bool is_maximisation) {
	...
	while (1) {
		...
		cout << "Outgoing variable: x" << tableau[leaving_var_row_index][1] - 1 << endl;
		...
		cout << "Incoming variable: x" << entering_var_col_index - 1 << endl;

		int num_var = num_cols - 3;
		vd variable_val(num_var, 0);
		for (int i = 0; i < num_rows; i++)	variable_val[tableau[i][1] - 2] = tableau[i][num_cols - 1];

		double optimization_value = find_dot_prod(objective_fn, variable_val);
...
vvd construct_dual_tableau(vvd mat, vd& objective_fn, bool& is_maximisation, vi& eqn_types) {
	int num_rows = mat.size();
	int num_cols = mat[0].size();
	...
	for (int i = 0; i < objective_fn.size(); i++) {
		ret_mat[i][num_rows] = objective_fn[i];
	}

	objective_fn.clear();
	for (int i = 0; i < num_rows; i++) {
		objective_fn.push_back(mat[i][num_cols - 1]);
	}
...
int main() {
	...
	int num_vars_original = n;
	vector<vector<double>> raw_mat(m, vector<double>(n + 1, 0));
	...
	convert_to_standard_form(raw_mat, eqn_types);
	bool is_problem_simplex = can_simplex_be_used(eqn_types);

	vvd mat = new_mat(raw_mat);
	...
	if (is_infeasible) {
		...
		//cout<<"Before: The coeeficient matrix: "<<endl;
		//printmat(raw_mat);
		//cout<<"Before: The objective function: "<<endl;
		//pritntvecd(objective_fn);
		make_vec(objective_fn, num_vars_original);
		//pritntvecd(objective_fn);
		//cout<<endl;

		vvd mat1 = construct_dual_tableau(raw_mat, objective_fn, is_maximisation, eqn_types);
		create_mat(eqn_types, mat1);
		int n_temp = mat1[0].size();
		create_maximisation_objective_function(objective_fn, is_maximisation, n_temp - 1);

		cout << "After: The coeeficient matrix: " << endl;
		printmat(mat1);
		cout << "After: The objective function: " << endl;
		pritntvecd(objective_fn);
		cout << endl;

		vvd tableau = construct_tableau(mat1);
		solve_with_dual_simplex(tableau, objective_fn, is_maximisation);
	}
}
