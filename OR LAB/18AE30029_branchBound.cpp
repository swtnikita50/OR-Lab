//Branch and Bound method for lab assignment 7
//Nikita
//18AE30029
...
int find_non_integer_index(vd variable_vals, int n) {
	int idx = -1;
	double p;
	for (int i = 0; i < n; i++) {
		//cout << "fractional part" << modf(variable_vals[i], &p) << endl;
		if (modf(variable_vals[i],&p) != 0) {
			idx = i + 1;
		}
	}
	//cout << "NON INTEGER INDEX IS:" << idx << endl;
	return idx;
}

vvd make_new_tableau_floor(vvd tableau, int idx, int x_idx_floor) {

	int col = tableau[0].size();
	int num_var = col - 3;
	int row = tableau.size();

	vd last_col(row, 0);
	for (int i = 0; i < row; i++) {
		last_col[i] = tableau[i][col - 1];
	}


	vvd new_tableau(row + 1, vd(col + 1, 0));
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			new_tableau[i][j] = tableau[i][j];
		}
	}
	//cout << endl << "Adding one slack variable..." << endl;
	for (int i = 0; i < col; i++) {
		new_tableau[row][i] = 0;
	}
	new_tableau[row][idx + 1] = 1;
	new_tableau[row][col - 1] = 1;
	for (int j = 0; j < row; j++) {
		new_tableau[j][col - 1] = 0;
		new_tableau[j][col] = last_col[j];
	}
	new_tableau[row][col] = x_idx_floor;
	new_tableau[row][1] = num_var + 2;
	//printmat(new_tableau);
	return new_tableau;
}

vvd make_new_tableau_ceil(vvd tableau, int idx, int x_idx_floor) {
	...
	vvd new_tableau(row + 1, vd(col + 2, 0));
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
			new_tableau[i][j] = tableau[i][j];
		}
	}
	//cout << "Adding one surplus and artificial variable..." << endl;
	for (int i = 0; i < col; i++) {
		new_tableau[row][i] = 0;
	}
	new_tableau[row][idx + 1] = 1;
	new_tableau[row][col - 1] = -1;
	new_tableau[row][col] = 1;
	for (int j = 0; j < row; j++) {
		new_tableau[j][col - 1] = 0;
		new_tableau[j][col] = 0;
		new_tableau[j][col + 1] = last_col[j];
	}
	new_tableau[row][col + 1] = x_idx_floor;
	new_tableau[row][1] = num_var + 3;
...
void solve_problem(vvd tableau, vd objective_fn, bool is_max, vi artificial_list, int n) {
	vvd initial_tableau = tableau;
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();
	...
	/////////////////////////////////////////////////////
	/////////////////////////////////////////////////////
	//BRANCH AND BOUND METHOD STARTING FROM HERE
	/////////////////////////////////////////////////////
	/////////////////////////////////////////////////////


	cout << endl << endl << "STARTING BRANCH AND BOUND..." << endl;

	vd variable_vals1 = variable_vals;
	if (find_non_integer_index(variable_vals1, n) != -1) {
		int idx = find_non_integer_index(variable_vals1, n);
		int x_idx_floor = floor(variable_vals1[idx - 1]);
		//cout << "FLOOR OF X_IDX" << x_idx_floor;
		int x_idx_ceil = ceil(variable_vals1[idx - 1]);
		vvd floor_tableau = make_new_tableau_floor(initial_tableau, idx, x_idx_floor);
		vvd ceil_tableau = make_new_tableau_ceil(initial_tableau, idx, x_idx_ceil);

		vd floor_obj_fn(objective_fn.size() + 1, 0);
		for (int i = 0; i < objective_fn.size(); i++) {
			floor_obj_fn[i] = objective_fn[i];
		}
		vd ceil_obj_fn(objective_fn.size() + 2, 0);
		for (int i = 0; i < objective_fn.size(); i++) {
			ceil_obj_fn[i] = objective_fn[i];
		}
		vi floor_artificial_list = artificial_list;
		vi ceil_artificial_list(artificial_list.size() + 1, 0);
		for (int i = 0; i < artificial_list.size(); i++) {
			ceil_artificial_list[i] = artificial_list[i];
		}
		ceil_artificial_list[artificial_list.size()] = ceil_obj_fn.size();

		/*cout << "Atrificial list:";
		for (int i = 0; i < ceil_artificial_list.size(); i++) {
			cout << ceil_artificial_list[i] << endl;
		}*/

		solve_problem(floor_tableau, floor_obj_fn, is_max, floor_artificial_list, n);
		solve_problem(ceil_tableau,  ceil_obj_fn, is_max, ceil_artificial_list, n);
...
int main() {
	...
	if (is_problem_simplex) {
		...
		printmat(tableau);
		solve_problem(tableau, objective_fn, is_maximisation, artificial_variable_list, n_fix);
	}
...
/*
INPUTS for QUESTIONS in LAB Assignment
Q1:
2
2
-2 3 6
