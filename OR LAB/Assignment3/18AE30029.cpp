...
void solve_with_simplex(vvd tableau, vd objective_fn, int iter){
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();
  ...
  cout << "Number of Iterations to solve the Problem: " << count << endl;
...
int main(){
	...
	solve_with_simplex(tableau, objective_fn, iter);
}
