// Use the BIG - M method to solve a linear optimisation problem with constraints of the form <= , >= and =
//Note: Using M_value as the large value of M
//Note: At the end are the question inputs that you can copy paste to see the output for the 5 questions 
...
void solve_problem(vvd tableau, vd objective_fn, bool is_max, vi artificial_list, int user_defined_iteration) {
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();
	...
	while (1) {
		...
		if (count == user_defined_iteration - 1) {
			...
			cout << endl << "Minimum Ratios: " << endl;
			for (int i = 0; i < num_rows; i++) {
				if (tableau[i][num_cols - 1] / tableau[i][entering_var_col_index] > 0) {
					cout << tableau[i][num_cols - 1] / tableau[i][entering_var_col_index] << ", ";
				}
			}
			cout << endl;
			cout << "Rest all xis=0 i.e. Non-Basic variables" << endl;
		}
	...
	if (user_defined_iteration > count + 1) {
		cout << "User Defined Iteration exceeds the number of iterations performed!" << endl;
	}
	...
	cout << endl << "Total Number of iterations to solve the Problem: " << count + 1 << endl;
...
int main() {
	int n, m, user_defined_iteration;
	...
	cout << endl << "Enter User Defined Iteration: " << endl;
	cin >> user_defined_iteration;
	...
	if (is_problem_simplex) {
		...
		printmat(tableau);
		solve_problem(tableau, objective_fn, is_maximisation, artificial_variable_list, user_defined_iteration);
	}
...
/*
QUESTION...COPY PASTE THESE INTO TERMINALS AS INPUTS
Q1:
4
2
1 2 3
