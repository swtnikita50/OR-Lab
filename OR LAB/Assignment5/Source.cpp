...
void create_maximisation_objective_function(vd& objective_fn, bool is_max, int n, vi slack_list, vi surplus_list, vi artificial_list) {
	...
	cout << "Total number of variables: " << n << endl;
	if (!is_max) {
...
void solve_with_simplex(vvd tableau, vd objective_fn, bool is_max) {
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();

	cout << endl << "Beginning Simplex method!!" << endl; int count = 0;
	while (1) {
		...
		for (int i = 0; i < num_rows; i++) {
			...
			if (tableau[i][entering_var_col_index] < zero_threshold) continue;
			if (tableau[i][num_cols - 1] < 0) continue;
		...
		tableau[leaving_var_row_index][1] = entering_var_col_index - 1;
	...
	for (int i = 0; i < num_rows; i++)	variable_vals[tableau[i][1] - 1] = tableau[i][num_cols - 1];

	cout << endl << "The Optimisation function is optimised at the point : ";
	for (int i = 0; i < num_rows; i++) {
		cout << "x" << tableau[i][1] << " = " << tableau[i][num_cols - 1] << ", ";
	}
	cout << "Rest all xis=0" << endl;

	double optimal_value = find_dot_prod(objective_fn, variable_vals);
	if (!is_max) optimal_value *= -1;
	cout << "The value of the objective function at this point is : " << optimal_value; cout << endl;
	//return;
...
bool is_in_vec(int index, vi list) {
	for (int i = 0; i < list.size(); i++) {
		if (index == list[i]) return 1;
	}
	return 0;
}

vd find_phase1_obj_fn(vi artificial_list, int num_vars) {
	vd ret(num_vars, 0);
	for (int i = 0; i < num_vars; i++) {
		if (is_in_vec(i + 1, artificial_list)) ret[i] = -1;
	}
	return ret;
}
...
vvd construct_bigM_tableau(vvd mat, vd objective_fn, vi slack_list, vi surplus_list, vi artificial_list) {
	...
	while ((slack_index < slack_list.size()) && (artificial_index < artificial_list.size())) {
		if (slack_list[slack_index] < artificial_list[artificial_index]) {
			tableau[curr_row][1] = slack_list[slack_index++];//column index in tableau
			tableau[curr_row++][0] = 0;
		}
		else {
			tableau[curr_row][1] = artificial_list[artificial_index++];
			tableau[curr_row++][0] = -1;
		}
	}
...
bool is_phase1_termination_reached(vi artificial_list, vi basis) {
	for (int i = 0; i < basis.size(); i++) {
		if (is_in_vec(basis[i], artificial_list)) return 0;
	}
	return 1;
}

void solve_phase_one(vvd& tableau, vd objective_fn, vi artificial_list) {
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();
	...
	while (1) {
		...
		vi basis;
		for (int i = 0; i < num_rows; i++) basis.pb((int)(tableau[i][1]));
		//cout << "Basis: "; pritntvec(basis);

		bool is_end = is_phase1_termination_reached(artificial_list, basis);
		if (is_end) { cout << "Reached the termination state of phase 1" << endl; break; }
...
vvd modify_tableau_for_phase2(vvd tableau, vi artificial_list, vd objective_fn) {
	int num_artificial_vars = artificial_list.size();
	int total_num_vars = tableau[0].size() - 3;
	int num_vars = total_num_vars - num_artificial_vars;
	vvd ret(tableau.size(), vd(num_vars + 3, 0));

	for (int i = 0; i < tableau.size(); i++) {
		int var_index = (int)tableau[i][1] - 1;
		ret[i][0] = objective_fn[var_index];
		ret[i][1] = tableau[i][1];
		ret[i][num_vars + 2] = tableau[i][total_num_vars + 2];
	}

	int curr_col_ret = 2;
	for (int i = 1; i <= total_num_vars; i++) {
		if (is_in_vec(i, artificial_list)) continue;
		else {
			for (int row = 0; row < tableau.size(); row++) {
				ret[row][curr_col_ret] = tableau[row][i + 1];
			}
			curr_col_ret++;
		}
	}
...
vd modify_objective_function_for_phase2(vd objective_fn, vi artificial_list) {
	vd ret;
	for (int i = 1; i <= objective_fn.size(); i++) {
		if (is_in_vec(i, artificial_list)) continue;
		ret.pb(objective_fn[i - 1]);
	}
	return ret;
}
...
int main() {
	...
	cout << endl << "The coeficient matrix: " << endl;
	printmat(mat);
	...
	if (is_problem_simplex) {
		...
		solve_with_simplex(tableau, objective_fn, is_maximisation);
	}

	else {
		int total_num_vars = mat[0].size() - 1;
		vd phase1_objective_fn = find_phase1_obj_fn(artificial_variable_list, total_num_vars);
		cout << "phase1_objective_fn: "; pritntvecd(phase1_objective_fn);
		//vvd tableau = construct_simplex_tableau(mat);
		vvd tableau = construct_bigM_tableau(mat, phase1_objective_fn, slack_variable_list, surplus_variable_list, artificial_variable_list);
		cout << "The initial tableau: " << endl;
		printmat(tableau);
		solve_phase_one(tableau, phase1_objective_fn, artificial_variable_list);
		tableau = modify_tableau_for_phase2(tableau, artificial_variable_list, objective_fn);
		objective_fn = modify_objective_function_for_phase2(objective_fn, artificial_variable_list);
		cout << "Phase2 matrix: " << endl; printmat(tableau);
		solve_with_simplex(tableau, objective_fn, is_maximisation);
		//solve_problem(tableau, phase1_objective_fn, is_maximisation, artificial_variable_list);
	}
	return 0;
}
