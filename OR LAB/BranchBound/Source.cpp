//Implementation of Gomory Cutting plane method to find integral solutions
//Nikita
//18AE30029
...
int find_non_integer_index(vd variable_vals, int n) {
	int idx = -1;
	double p;
	//int n = variable_vals.size();
	for (int i = 0; i < n; i++) {
		//cout << "fractional part" << modf(variable_vals[i], &p) << endl;
		if (modf(variable_vals[i], &p) > epsilon_threshold) {
			//cout << "variable value for" << i << ":" << variable_vals[i];
			idx = i + 1;
		}
	}
...
bool are_all_vars_int(vd variable_vals) {
	...
	for (int i = 0; i < variable_vals.size(); i++) {
		...
		if (modf(variable_vals[i], &p) != 0) return 0;
	}
	return 1;
}
...
vvd make_new_tableau(int eqn_index, vvd tableau) {
	...
	//cout << "INSIDE MAKE NEW TABLEAU:" << endl;
...
void solve_with_dual_simplex(vvd& tableau, vd& objective_fn, bool is_maximisation, bool& is_valid_integral_found, vd& variable_vals, int n_init) {
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();

	//cout<<endl<<endl<<"Beginning Dual Simplex method!!"<<endl;
	int count = 0;
	while (1) {
	...
	//cout << "Value of find_non_integer_index:" << find_non_integer_index(variable_vals, n_init) << endl;
	if (find_non_integer_index(variable_vals, n_init) == -1) {
...
vd update_obj_fn(vd objective_fn, vvd tableau) {

	int cols = tableau[0].size();
	int new_vars = cols - 3;
	int old_vars = objective_fn.size();
	vd new_obj_fn(new_vars, 0);
	for (int i = 0; i < old_vars; i++) {
		new_obj_fn[i] = objective_fn[i];
	}
...
vd update_variable_vals(vd variable_vals, vvd tableau) {
	int cols = tableau[0].size(), rows = tableau.size();
	int new_vars = cols - 3;
	int old_vars = variable_vals.size();
	int k;
	vd new_var_val(new_vars, 0);
	//printmat(tableau);
	for (int i = 0; i < rows; i++) {
		k = round(tableau[i][1]);
		//cout << k;
		new_var_val[k - 2] = tableau[i][cols - 1];
		//cout << "NEW_VARIABLE_VALS:" << new_var_val[k - 2] << endl;
	}

	//for (int i = 0; i < new_var_val.size(); i++) {
		//cout << "NEW_VARIABLE_VALS:" << new_var_val[i] << endl;
	//}
...
void solve_problem(vvd tableau, vd objective_fn, bool is_max, vi artificial_list, int n_init) {
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();
	...
	vd objective_fn1, variable_vals1;
	variable_vals1 = variable_vals;
	while (iteration_index < 5) {
		cout << endl << endl << "Iteration index: " << iteration_index << endl;
		eqn_index = find_eqn_index_of_var_with_max_fractional(variable_vals1, tableau);
		tableau = make_new_tableau(eqn_index, tableau);
		objective_fn1 = update_obj_fn(objective_fn, tableau);
		variable_vals1 = update_variable_vals(variable_vals, tableau);


		//cout << "Equation Index of variable with max fractional:" << eqn_index << endl;

		//cout<<"After updating matrix, new matrix: "<<endl;
		//printmat(tableau);
		...
		solve_with_dual_simplex(tableau, objective_fn1, is_max, is_valid_integral_found, variable_vals1, n_init);
		//cout << "Value of is_valid_integral_found:" << is_valid_integral_found << endl;
...
int main() {
	...
	vvd tableau = construct_bigM_tableau(mat, objective_fn, slack_variable_list, surplus_variable_list, artificial_variable_list);
	solve_problem(tableau, objective_fn, is_maximisation, artificial_variable_list, n_init);
