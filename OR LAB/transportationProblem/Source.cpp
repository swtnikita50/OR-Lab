...
/*vvi VAM(vvi tableau, vvi* mat2) {
	...
	//compute penalties for every row
	vi row_penalties(num_rows - 1);
	vi col_penalties(num_cols - 1);
	for (int i = 0; i < num_rows-1; i++) {
		vi row(num_cols - 1);
		for (int j = 0; j < num_cols - 1; j++) {
			row[j] = tableau[i][j];
		}
		row_penalties[i] = find_penalty(row);
	}
	for (int i = 0; i < num_cols - 1; i++) {
		...
		col_penalties[i] = find_penalty(col);
	}

	vi max_row_penalty = find_max(row_penalties);//index, value
	vi max_col_penalty = find_max(col_penalties);//index, value

	if (max_col_penalty[1] > max_row_penalty[1]) {
		//find cell with min cost in the column
		vi col(num_rows - 1);
		for (int j = 0; j < num_rows - 1; j++) {
			col[j] = tableau[j][max_col_penalty[0]];
		}
		vi min_cost_cell = find_min(col);
		mat2[min_cost_cell[0]][max_col_penalty[0]] = min(tableau[num_rows-1][max_col_penalty[0]], tableau[min_cost_cell[0]][num_cols-1]);
		vvi tableau1 = column_striked_tableau(tableau, max_col_penalty[0]);
		VAM(tableau1, &mat2);
	}

	else{
		//find cell with min cost in the row
		vi row(num_cols - 1);
		for (int j = 0; j < num_cols - 1; j++) {
			row[j] = tableau[max_row_penalty[0]][j];
		}
		vi min_cost_cell = find_min(row);
		mat2[min_cost_cell[0]][max_row_penalty[0]] = min(tableau[max_row_penalty[0]][num_cols-1], tableau[num_rows-1][min_cost_cell[0]]);
		vvi tableau2 = row_striked_tableau(tableau, max_row_penalty[0]);
		VAM(tableau2, &mat2);
	}
...
vvi MatrixMinimaMethod(vvi tableau) {
	int curr_row = 0;
	int curr_col = 0;
	vvi mat(num_origins, vector<int>(num_destinations, -1));
...
void solve_using_uv_method(vvi tableau, vvi mat, bool method) {
	...
	while (1) {
		...
		cout << "Optimality NOT yet reached!" << endl;
...
int main() {
	...
	cout << "Finding Initial BFS using NorthWest Corner Method..." << endl;

	vvi mat = NW_corner_method(tableau);
	cout << endl << "Initial table(-1 represent unallocated cells) -> " << endl;
	print_mat(mat);
	cout << endl << "Current Cost:" << find_cost(mat, tableau) << endl << endl;

	cout << "Finding Initial BFS using Matrix Minima Method..." << endl;

	vvi mat1 = MatrixMinimaMethod(tableau);
	cout << endl << "Initial table(-1 represent unallocated cells) -> " << endl;
	print_mat(mat1);
	cout << endl << "Current Cost:" << find_cost(mat1, tableau) << endl << endl;

	/*cout << "Finding Initial BFS using Vogle's Approximation Method..." << endl;
	
	vvi mat2(num_origins, vector<int>(num_destinations, -1));;
	mat2 = VAM(tableau,&mat2);
	cout << endl << "Initial table(-1 represent unallocated cells) -> " << endl;
	print_mat(mat2);
	cout << endl << "Current Cost:" << find_cost(mat2, tableau) << endl << endl;*/
